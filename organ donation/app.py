...
def sidebar_information():
    ...
    st.sidebar.image("Images/s4.png", caption="Domain Overview", use_container_width=True)
    st.sidebar.markdown("""
    ...
    st.sidebar.image("Images/s2.png", caption="Domain Overview", use_container_width=True)
    st.sidebar.markdown("""
    ...
    st.sidebar.image("Images/s3.png", caption="Domain Overview", use_container_width=True)
    st.sidebar.markdown("""
...
def register_user():
    ...
    st.image("Images/d4.png", use_container_width=True)
    ...
    if st.button("Register"):
        ...
        with open(FILE_NAME, mode='a', newline='') as file:
            ...
            writer.writerow([user_id, name, age, blood_type, ",".join(organs), contact_info, hashed_password,
                             location,Hospital_Name, chronic_diseases, infectious_diseases, cancer_history, current_medications])
...
def request_organ():
    if 'logged_in' in st.session_state and st.session_state.logged_in:
        ...
        st.image("Images/d5.png", use_container_width=True)
        ...
        if st.button("Search for Match"):
            ...
            if sorted_matches:
                ...
                for match in sorted_matches:
                    ...
                    st.write("Hospital Name:", match[8])
                    st.write("Chronic Diseases:", match[9])
                    st.write("Infectious Diseases:", match[10])
                    st.write("Cancer History:", match[11])
                    st.write("Current Medications:", match[12])
                    st.write("---")
            else:
...
def login_blockchain_admin():
    # Initialize session state for login
    if 'logged_in_blockchain' not in st.session_state:
        st.session_state.logged_in_blockchain = False
    if 'is_blockchain_admin' not in st.session_state:
        st.session_state.is_blockchain_admin = False

    # Check if the blockchain admin is already logged in
    if st.session_state.is_blockchain_admin:
        st.success("You are already logged in as Blockchain Admin.")
        try:
            # Load and display the blockchain data
            with open("blockchain_data.json", "r") as f:
                blockchain_data = json.load(f)
            st.json(blockchain_data)  # Display blockchain data
        except FileNotFoundError:
            st.error("blockchain_data.json not found.")
        return  # Skip blockchain admin login UI if already logged in

    st.title("Blockchain Admin Login")
    st.image("Images/d6.png", use_container_width=True)


    name = st.text_input("Name")
    password = st.text_input("Password", type="password")

    # Hardcoded credentials for blockchain admin
    correct_username_blockchain = "blockchain_admin"
    correct_password_blockchain = "blockchain_password"

    if st.button("Login"):
        if name == correct_username_blockchain and password == correct_password_blockchain:
            st.session_state.logged_in_blockchain = True
            st.session_state.is_blockchain_admin = True  # Mark as blockchain admin
            st.session_state.user_name = name  # Store the username in session state
            st.success(f"Login Successful! Welcome {name}")

            # Load and display the blockchain data upon successful login
            try:
                with open("blockchain_data.json", "r") as f:
                    blockchain_data = json.load(f)
                st.json(blockchain_data)  # Display blockchain data
...
def main():
    ...
    choice = st.sidebar.selectbox("Select Option", ["Register", "Login", "Admin","Blockchain Admin"])
    ...
    elif choice == "Blockchain Admin":
        login_blockchain_admin()
